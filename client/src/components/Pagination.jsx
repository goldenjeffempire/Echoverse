...
export function Pagination({ currentPage, totalPages, onPageChange, showFirstLast = true, maxVisible = 5, className = '', }) {
    ...
    return (<nav className={`flex items-center justify-center space-x-2 ${className}`}>
      ...
      {getPageNumbers().map((page, index) => (<div key={index}>
          {page === '...' ? (<MoreHorizontal className="h-4 w-4 text-muted-foreground"/>) : (<Button variant={currentPage === page ? 'default' : 'outline'} size="sm" onClick={() => onPageChange(page)}>
              {page}
            </Button>)}
        </div>))}
