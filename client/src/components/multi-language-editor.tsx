...
export function MultiLanguageEditor() {
  ...
  const loadContent = async () => {
    setLoading(true)
    try {
      const response = await apiClient.get<{ content: ContentItem[] }>('/api/content/translations')
  ...
  const calculateProgress = (item: ContentItem, language: string) => {
  ...
  const handleSaveTranslation = async (item: ContentItem, language: string, field: string, value: string) => {
    setSaving(true)
    try {
  ...
  const handleAutoTranslate = async (item: ContentItem, targetLanguage: string) => {
    try {
  ...
  const handleCopyFromBase = (item: ContentItem, targetLanguage: string) => {
    const baseTranslation = item.translations.find(t => t.language === item.baseLanguage)
                ...
                {SUPPORTED_LANGUAGES.map(lang => (
                  <SelectItem key={lang.code} value={lang.code}>
                    <span className="flex items-center gap-2">
                      <span>{lang.flag}</span>
                      <span>{lang.name}</span>
                      {lang.code === selectedItem.baseLanguage && (
            ...
            {selectedLanguage !== selectedItem.baseLanguage && (
              <div className="mt-4 flex gap-2">
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => handleCopyFromBase(selectedItem, selectedLanguage)}
                  className="flex-1"
                >
                ...
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => handleAutoTranslate(selectedItem, selectedLanguage)}
                  className="flex-1"
                >
        ...
        <CardContent>
          <Tabs defaultValue="content" className="space-y-4">
            ...
            <TabsContent value="content" className="space-y-4">
              {baseTranslation && Object.keys(baseTranslation.content).map(field => (
                <div key={field} className="space-y-2">
                  <label className="text-sm font-medium capitalize flex items-center gap-2">
                    {field.replace(/_/g, ' ')}
                    {currentTranslation?.content[field] && (
                      <Check className="h-4 w-4 text-green-600" />
                    )}
                  </label>
                  <Textarea
                    value={currentTranslation?.content[field] || ''}
                    onChange={(e) => handleSaveTranslation(selectedItem, selectedLanguage, field, e.target.value)}
                    placeholder={`Enter ${field} in ${SUPPORTED_LANGUAGES.find(l => l.code === selectedLanguage)?.name}`}
              ...
              {baseTranslation && Object.keys(baseTranslation.content).map(field => (
                <div key={field} className="grid gap-4 md:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <Textarea
                      value={baseTranslation.content[field] || ''}
                      disabled
