...
import { Rss, Plus, Copy, Download, RefreshCw, Trash2, ExternalLink } from "lucide-react";
...
export function RssFeedManager() {
    ...
    const handleCreateFeed = async () => {
        ...
        try {
            const response = await apiClient.post('/api/rss/feeds', newFeed);
    ...
    const handleGenerateFeed = async (feedId) => {
        setGenerating(true);
        try {
      ...
      <Tabs defaultValue="feeds" className="space-y-6">
        ...
        <TabsContent value="feeds" className="space-y-4">
          {feeds.length === 0 ? (<Card>
            ...
            </Card>) : (feeds.map(feed => (<Card key={feed.id}>
                <CardHeader>
                  <div className="flex items-start justify-between">
                    ...
                    <Switch checked={feed.enabled} onCheckedChange={(checked) => handleToggleFeed(feed.id, checked)}/>
                  </div>
                </CardHeader>
                <CardContent className="space-y-4">
                  ...
                  <div className="flex items-center justify-between text-sm text-muted-foreground">
                    <div className="flex gap-4">
                      ...
                      {feed.lastGenerated && (<span>Last generated: {new Date(feed.lastGenerated).toLocaleDateString()}</span>)}
                    </div>
                  </div>

                  <div className="flex gap-2">
                    <Button size="sm" variant="outline" onClick={() => handleGenerateFeed(feed.id)} disabled={generating}>
                      <RefreshCw className="h-4 w-4 mr-2"/>
                      Regenerate
                    </Button>
                    ...
                    <Button size="sm" variant="destructive" onClick={() => handleDeleteFeed(feed.id)}>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <Input id="title" value={newFeed.title} onChange={(e) => setNewFeed({ ...newFeed, title: e.target.value })} placeholder="My Awesome Blog Feed"/>
              </div>

              <div className="space-y-2">
                <Label htmlFor="description">Feed Description *</Label>
                <Textarea id="description" value={newFeed.description} onChange={(e) => setNewFeed({ ...newFeed, description: e.target.value })} placeholder="Latest updates from my blog"/>
              </div>

              <div className="space-y-2">
                <Label htmlFor="contentType">Content Type</Label>
                <Select value={newFeed.contentType} onValueChange={(value) => setNewFeed({ ...newFeed, contentType: value })}>
                  <SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                <Input id="itemLimit" type="number" min="1" max="100" value={newFeed.itemLimit} onChange={(e) => setNewFeed({ ...newFeed, itemLimit: parseInt(e.target.value) || 20 })}/>
              ...
              <div className="flex items-center justify-between">
                ...
                <Switch checked={newFeed.includeImages} onCheckedChange={(checked) => setNewFeed({ ...newFeed, includeImages: checked })}/>
              </div>
