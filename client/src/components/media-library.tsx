...
export function MediaLibrary() {
  ...
  const loadMediaFiles = async () => {
    setLoading(true)
    try {
      const response = await apiClient.get<{ files: MediaFile[] }>('/api/media')
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const response = await apiClient.post<{ files: MediaFile[] }>('/api/upload-multiple', formData, {
  ...
  const getFileIcon = (type: string) => {
    ...
    if (type.startsWith('application')) return <FileText className="h-4 w-4" />
        ...
        <CardContent className="pt-6">
          <div className={viewMode === "grid" ? "grid gap-4 md:grid-cols-3 lg:grid-cols-4" : "space-y-2"}>
            {filteredFiles.map(file => (
              <div
                key={file.id}
                className={`relative group border rounded-lg p-3 hover-elevate cursor-pointer ${
                      ...
                      <Dialog>
                        ...
                        <DialogContent>
                          ...
                          {file.type.startsWith('image') && (
                            <img src={file.url} alt={file.name} className="max-h-96 w-full object-contain" />
                          )}
                        </DialogContent>
                      </Dialog>
