...
import { Package, Star, Download, DollarSign, Search, Filter, Plus, Code, Shield, TrendingUp } from "lucide-react";
export function Marketplace() {
    ...
    const renderStarRating = (rating) => {
        ...
        {[...Array(5)].map((_, i) => (<Star key={i} className={`h-3 w-3 ${i < Math.floor(rating) ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300'}`}/>))}
      ...
      <Tabs defaultValue="browse" className="space-y-6">
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {plugins.map((plugin) => (<Card key={plugin.id} className="hover-elevate" data-testid={`plugin-card-${plugin.id}`}>
                    <CardHeader className="pb-4">
                      <div className="flex items-start justify-between">
                        ...
                        {plugin.rating >= 4.5 && (<Badge variant="secondary">Featured</Badge>)}
                      </div>
                    </CardHeader>
                    <CardContent className="space-y-4">
                      ...
                      <Button className="w-full" data-testid={`button-install-${plugin.id}`}>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                {myPlugins.map((plugin) => (<div key={plugin.id} className="flex items-center justify-between p-4 border rounded-lg hover-elevate" data-testid={`installed-plugin-${plugin.id}`}>
        ...
        <TabsContent value="categories" className="space-y-6">
          <Card>
            ...
            <CardContent>
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {categories.map((category) => (<Card key={category.name} className="hover-elevate cursor-pointer" data-testid={`category-${category.name.toLowerCase()}`}>
