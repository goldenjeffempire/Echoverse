...
export function CommentModeration() {
  ...
  const loadComments = async () => {
    setLoading(true)
    try {
      const response = await apiClient.get<{ comments: Comment[] }>('/api/comments/moderate')
  ...
  const handleReject = async (commentId: string) => {
    try {
      ...
      setComments(comments.map(c => c.id === commentId ? { ...c, status: "rejected" as const } : c))
      toast({
  ...
  const handleMarkAsSpam = async (commentId: string) => {
    try {
      ...
      setComments(comments.map(c => c.id === commentId ? { ...c, status: "spam" as const } : c))
      toast({
    ...
    try {
      ...
      if (action === "delete") {
        ...
        setComments(comments.map(c => 
          selectedComments.has(c.id) ? { ...c, status: action as Comment['status'] } : c
        ))
      }
