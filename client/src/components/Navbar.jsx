...
import { Sparkles, Menu, ChevronDown, Globe, ShoppingCart, Puzzle, TrendingUp, BookOpen, Code, Users, MessageSquare, Zap } from "lucide-react";
...
const Navbar = () => {
    ...
    const DropdownMenu = ({ title, items }) => {
        ...
        return (<div className="relative" onMouseEnter={() => setActiveDropdown(title)} onMouseLeave={() => setActiveDropdown(null)}>
        ...
        <AnimatePresence>
          {isActive && (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} transition={{ duration: 0.2 }} className="absolute top-full left-0 mt-2 w-72 bg-white dark:bg-slate-900 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-700 p-2 z-50">
    ...
    return (<>
      <motion.nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled
            ? 'bg-white/95 dark:bg-slate-900/95 backdrop-blur-lg shadow-lg'
            : 'bg-transparent'}`} initial={{ y: -100 }} animate={{ y: 0 }} transition={{ duration: 0.3 }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            {/* Logo */}
            <motion.button onClick={() => setLocation("/")} className="flex items-center gap-2 group" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
            ...
            <div className="hidden lg:flex items-center gap-1">
              ...
              <button onClick={() => setLocation("/about")} className="px-4 py-2 text-sm font-medium text-slate-700 dark:text-slate-200 hover:text-blue-600 dark:hover:text-blue-400 transition-colors">
                Solutions
              </button>
              <DropdownMenu title="Resources" items={resourceItems}/>
              <button onClick={() => setLocation("/pricing")} className="px-4 py-2 text-sm font-medium text-slate-700 dark:text-slate-200 hover:text-blue-600 dark:hover:text-blue-400 transition-colors">
                Pricing
              </button>
            </div>
            ...
            <div className="hidden lg:flex items-center gap-3">
              {user ? (<>
                  <Button variant="ghost" onClick={() => setLocation("/dashboard")} className="font-medium">
                    Dashboard
                  </Button>
                  <Button onClick={() => setLocation("/ai-builder")} className="bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white font-semibold shadow-lg">
                    Start Building
                  </Button>
                </>) : (<>
                  <Button variant="ghost" onClick={() => setLocation("/login")} className="font-medium">
                    Sign In
                  </Button>
                  <Button onClick={() => setLocation("/register")} className="bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white font-semibold shadow-lg">
                    Get Started
                  </Button>
                </>)}
            </div>
              ...
              <SheetContent side="right" className="w-full sm:w-96">
                <div className="flex flex-col gap-6 mt-8">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      {productItems.map((item, index) => (<button key={index} onClick={() => {
                setLocation(item.href);
                setMobileMenuOpen(false);
                    ...
                    <div>
                      ...
                      {resourceItems.map((item, index) => (<button key={index} onClick={() => {
                setLocation(item.href);
                setMobileMenuOpen(false);
            }} className="w-full flex items-center gap-3 p-3 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                          <item.icon className="w-5 h-5 text-blue-600 dark:text-blue-400"/>
