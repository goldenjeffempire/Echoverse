...
export function CreatorHub() {
  return (
    <section className="py-32 px-4 sm:px-6 lg:px-8 bg-gradient-to-br from-slate-50 via-purple-50 to-blue-50 dark:from-slate-900 dark:via-purple-950 dark:to-blue-950 relative overflow-hidden" data-testid="section-creator-hub">
      {/* Background Elements */}
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid md:grid-cols-3 gap-8 mb-16">
          {creators.map((creator, index) => (
            <motion.div
              ...
              whileHover={{ y: -10 }}
              data-testid={`card-creator-${creator.name.toLowerCase().replace(/\s+/g, '-')}`}
            >
              <Card className="relative overflow-hidden h-full border-2 border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 hover:shadow-2xl transition-shadow duration-500">
                ...
                <motion.div
                  className={`absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r ${creator.color}`}
                  initial={{ x: "-100%" }}
                  whileHover={{ x: "100%" }}
        ...
        >
          ...
          <motion.button
            className="px-10 py-5 bg-gradient-to-r from-purple-600 via-pink-600 to-blue-600 text-white font-bold rounded-full text-lg shadow-2xl"
            whileHover={{ scale: 1.05, boxShadow: "0 20px 60px rgba(139, 92, 246, 0.5)" }}
            whileTap={{ scale: 0.95 }}
            data-testid="button-join-creators"
          >
