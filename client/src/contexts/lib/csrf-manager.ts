...
type CSRFState = 'idle' | 'initializing' | 'ready' | 'failed';

class CSRFTokenManager {
  private state: CSRFState = 'idle';
  private initPromise: Promise<boolean> | null = null;
  private failureCount: number = 0;
  private lastFailureTime: number = 0;
  private readonly MAX_RETRIES = 5;
  private readonly FAILURE_COOLDOWN_MS = 30000; // 30 seconds before allowing retry after total failure
   ...
   */
  private async fetchTokenWithRetry(attempt: number = 1): Promise<boolean> {
    try {
   ...
   */
  async initialize(options: { force?: boolean } = {}): Promise<boolean> {
