...
export default function SearchPage() {
    ...
    const performSearch = async (searchQuery) => {
        ...
        try {
            const [products, posts, communities] = await Promise.all([
                api.get('/products', { params: { search: searchQuery, limit: 10, offset: 0 } }).catch(() => ({ data: [] })),
                api.get('/posts', { params: { search: searchQuery, limit: 10, offset: 0 } }).catch(() => ({ data: [] })),
                api.get('/communities', { params: { search: searchQuery, limit: 10, offset: 0 } }).catch(() => ({ data: [] })),
            ]);
            const allResults = [
                ...(products.data || []).map((p) => ({
                    id: String(p.id || ''),
                ...
                ...(posts.data || []).map((p) => ({
                    ...
                    description: String(p.content?.substring(0, 200) || ''),
                    url: `/posts/${p.slug}`,
                    metadata: p,
                })),
    ...
    return (<div className="max-w-4xl mx-auto space-y-6">
        ...
        <CardContent className="pt-6">
          <form onSubmit={handleSearch} className="flex gap-2">
            <div className="relative flex-1">
              ...
              <Input placeholder="Search for anything..." value={query} onChange={(e) => setQuery(e.target.value)} className="pl-10 text-lg" autoFocus/>
            </div>
      ...
      {results.length > 0 && (<Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList>
          ...
          <TabsContent value={activeTab} className="space-y-4 mt-6">
            {filteredResults.map((result) => (<Card key={`${result.type}-${result.id}`} className="hover:shadow-md transition-shadow">
                <CardHeader>
      ...
      {!loading && query && results.length === 0 && (<Card>
