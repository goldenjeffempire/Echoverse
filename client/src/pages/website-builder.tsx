...
function SortableComponent({ component, index, onEdit, onDelete }: SortableComponentProps) {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  return (
    <div
      ...
      {...attributes}
      {...listeners}
      className="border border-gray-200 rounded-lg p-4 bg-white shadow-sm hover:shadow-md transition-shadow cursor-grab active:cursor-grabbing"
    >
...
export default function WebsiteBuilder() {
  const [currentView, setCurrentView] = useState<'design' | 'code' | 'preview'>('design');
  const [selectedPage, setSelectedPage] = useState<string>('');
  const [websiteData, setWebsiteData] = useState<WebsiteData | null>(null);
  const [isGenerating, setIsGenerating] = useState(false);
  const [activePanel, setActivePanel] = useState<'ai' | 'components' | 'templates' | 'settings'>('ai');
  ...
  const handleGenerateComponent = useCallback(async (type: string, description: string) => {
    try {
      const token = localStorage.getItem('accessToken');
      if (!token) {
              ...
              >
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Describe what kind of website you want..."
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setGenerationParams(prev => ({ ...prev, businessType: e.target.value }))}
                ...
                <div>
                  ...
                  <select
                    value={generationParams.colorScheme}
                    onChange={(e) => setGenerationParams(prev => ({ ...prev, colorScheme: e.target.value }))}
                    className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  >
                ...
                <button
                  ...
                  disabled={isGenerating || !generationParams.description || !generationParams.businessType}
