...
export default function Forums() {
  ...
  const loadForumData = async () => {
    ...
    try {
      const [threadsRes, categoriesRes] = await Promise.all([
        apiClient.get<{ threads: ForumThread[] }>('/api/forum/threads'),
        apiClient.get<{ categories: ForumCategory[] }>('/api/forum/categories')
      ])
  ...
  const handleCreateThread = async () => {
    ...
    try {
      const response = await apiClient.post<{ thread: ForumThread }>('/api/forum/threads', newThread)
          ...
          <div className="space-y-3">
            {filteredThreads.length === 0 ? (
              ...
              filteredThreads.map(thread => (
                <Card key={thread.id} className="hover-elevate cursor-pointer">
                  <CardHeader>
