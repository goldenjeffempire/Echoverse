      ...
      <section className="pt-32 pb-20 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-8 mb-16">
            {pricingPlans.map((plan, index) => (<motion.div key={index} initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className="relative">
                ...
                <Card className={`h-full border-0 shadow-2xl ${plan.popular ? 'ring-2 ring-purple-500 scale-105' : ''} hover:shadow-3xl transition-all`}>
                  <CardContent className="p-8">
                    ...
                    <Button className={`w-full mb-8 bg-gradient-to-r ${plan.color} text-white hover:opacity-90`} size="lg" onClick={() => setLocation("/register")}>
                      {plan.cta}
                    </Button>
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="bg-gradient-to-br from-blue-600 to-purple-600 rounded-3xl p-12 text-center text-white">
            ...
            <Button size="lg" className="bg-white text-blue-600 hover:bg-blue-50 px-8" onClick={() => setLocation("/contact")}>
              Contact Sales
            </Button>
          </motion.div>
        </div>
