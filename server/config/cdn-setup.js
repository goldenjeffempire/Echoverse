 ...
 * Upload asset to CDN
 */
export async function uploadAssetToCDN(localPath, cdnPath) {
    if (!CDN_CONFIG.enabled) {
        return { success: false, error: 'CDN not enabled' };
    }
    try {
        const fileContent = readFileSync(localPath);
        const contentType = ASSET_TYPES[extname(localPath)] || 'application/octet-stream';
 ...
 */
export async function uploadDistToCDN(distDir) {
 ...
 */
export async function invalidateCDN(paths = ['/*']) {
    if (!CDN_CONFIG.enabled) {
