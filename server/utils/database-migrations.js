 ...
 */
async function preValidateSchema() {
    try {
        const result = await pool.query(`
 ...
 */
async function postValidateDataIntegrity(migrationVersion) {
    try {
        const result = await pool.query(`
 ...
 */
async function verifyRollback(migration) {
    if (!migration.down) {
...
export async function applyMigration(migration) {
    ...
    try {
        ...
        try {
            ...
            await pool.query(`INSERT INTO migrations (id, name, version, checksum, execution_time_ms) 
         VALUES ($1, $2, $3, $4, $5)`, [migration.id, migration.name, migration.version, checksum, executionTime]);
 ...
 */
async function validateRollbackSafety(migration) {
    const warnings = [];
    if (!migration.down) {
 ...
 */
export async function rollbackMigration(migration, options = {}) {
    try {
        ...
        try {
            ...
            await pool.query('DELETE FROM migrations WHERE version = $1', [migration.version]);
            await pool.query('COMMIT');
