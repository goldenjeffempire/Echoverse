 ...
 */
function getBackupEncryptionKey(version) {
 ...
 */
export async function encryptBackupFile(inputPath, outputPath) {
    try {
 ...
 */
export async function decryptBackupFile(encryptedPath, outputPath, keyVersion) {
    try {
 ...
 */
export async function createEncryptedBackup(outputDir = './backups') {
    ...
    try {
        ...
        const { stdout, stderr } = await execAsync(`pg_dump "${DATABASE_URL}" --no-password --clean --if-exists > "${backupPath}"`, { maxBuffer: 50 * 1024 * 1024 } // 50MB buffer
        );
