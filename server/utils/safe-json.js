...
export function safeJSONParse(jsonString, options = {}) {
    ...
    if (!jsonString || typeof jsonString !== 'string') {
        ...
        throw new AIServiceError(`Invalid JSON response from AI service: Expected string, got ${typeof jsonString}`, 500);
    }
    try {
        ...
        if (schema && !schema(parsed)) {
            ...
            throw new AIServiceError('AI response does not match expected schema', 500);
        }
        return parsed;
    }
    catch (error) {
        ...
        throw new AIServiceError(`Failed to parse AI response: ${errorMessage}. The AI service may have returned invalid JSON.`, 500);
    }
}
/**
 * Validates that parsed JSON contains required fields
 */
export function validateJSONFields(data, requiredFields, context = 'JSON validation') {
    ...
    if (missingFields.length > 0) {
        ...
        throw new AIServiceError(`AI response missing required fields: ${missingFields.join(', ')}`, 500);
    }
    return data;
}
/**
 ...
 */
export function extractJSON(content, options = {}) {
 ...
 */
export function safeJSONStringify(data, options = {}) {
    const { pretty = false, maxLength } = options;
    try {
        const stringified = JSON.stringify(data, null, pretty ? 2 : undefined);
