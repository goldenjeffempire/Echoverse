...
export async function initializeAPIKeysTable() {
...
export async function rotateAPIKey(oldKeyId, userId) {
  ...
  `);
    ...
    const oldKeyData = oldKey.rows[0];
    const { key, hash, prefix } = generateAPIKey();
    const expiresAt = new Date(Date.now() + KEY_EXPIRY_DAYS * 24 * 60 * 60 * 1000);
    const result = await db.execute(sql `
...
export async function checkExpiringKeys(daysThreshold = 7) {
