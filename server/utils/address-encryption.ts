...
export interface EncryptedAddress {
  ...
  _encrypted: true; // Flag to indicate this is encrypted
}

/**
 * Encrypt an address object for storage
 */
export function encryptAddress(address: Address | null | undefined): EncryptedAddress | null {
 ...
 */
export function decryptAddress(encryptedAddress: EncryptedAddress | Address | null | undefined): Address | null {
  if (!encryptedAddress) {
    return null;
  }
  ...
  if (!('_encrypted' in encryptedAddress) || !encryptedAddress._encrypted) {
    return encryptedAddress as Address;
  }

  try {
    const decrypted = decryptSensitiveField(encryptedAddress.encryptedData);
    return JSON.parse(decrypted) as Address;
  } catch (error) {
...
export function encryptAddresses(addresses: {
  ...
  shipping: EncryptedAddress | null;
  billing: EncryptedAddress | null;
} {
  return {
...
export function decryptAddresses(addresses: {
  shipping?: EncryptedAddress | Address | null;
  billing?: EncryptedAddress | Address | null;
}): {
  shipping: Address | null;
  billing: Address | null;
} {
 ...
 */
export function maskAddress(address: Address | null | undefined): Partial<Address> | null {
