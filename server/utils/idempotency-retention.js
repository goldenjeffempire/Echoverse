...
export async function cleanupIdempotencyKeys() {
    ...
    try {
        ...
        const paidResult = await db
            ...
            .where(and(lte(orders.createdAt, paidCutoff), sql `${orders.paymentStatus} = 'paid'`, isNotNull(orders.idempotencyKey)));
        // Clear idempotency keys from old failed/cancelled orders
        const failedResult = await db
            .update(orders)
            .set({ idempotencyKey: null })
            .where(and(lte(orders.createdAt, failedCutoff), sql `${orders.paymentStatus} IN ('failed', 'cancelled')`, isNotNull(orders.idempotencyKey)));
        // Clear idempotency keys from old payment intents
        const paymentIntentResult = await db
            .update(paymentIntents)
            .set({ idempotencyKey: null })
            .where(and(lte(paymentIntents.createdAt, paymentIntentCutoff), isNotNull(paymentIntents.idempotencyKey)));
 ...
 */
export function startIdempotencyCleanup(intervalMs = 3600000) {
