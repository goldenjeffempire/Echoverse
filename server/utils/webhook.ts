...
export function generateWebhookSignature(payload: string | Buffer, secret?: string): string {
...
export function createWebhookPayload(event: string, data: any): WebhookPayload {
  return {
    event,
    data,
    timestamp: Date.now(),
...
export function signWebhookPayload(payload: WebhookPayload, secret?: string): {
  payload: string;
  signature: string;
...
export async function checkWebhookReplayProtection(
  eventId: string,
  eventType: string,
  payload?: any
): Promise<boolean> {
 ...
 */
export async function cleanupOldWebhookEvents(daysToKeep?: number): Promise<number> {
  ...
  try {
    ...
    const result = await db
      ...
      .where(
        drizzleSql`${webhookEvents.createdAt} < ${cutoffDate} AND ${webhookEvents.processed} = true`
      )
