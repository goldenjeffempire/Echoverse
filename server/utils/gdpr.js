...
export class GDPRService {
    static async exportUserData(userId, requesterIp) {
        try {
            const [user] = await db.select().from(users).where(eq(users.id, userId));
            if (!user) {
                throw new Error('User not found');
            }
            const [userWebsites, userPosts, userOrders, userCommunities, userMessages] = await Promise.all([
    ...
    static async deleteUserData(userId, requesterIp) {
        try {
    ...
    static async getDataRetentionInfo(userId) {
        return {
            userId,
            retentionPeriods: {
