...
class KeyRotationManager {
  private keys: Map<string, KeyRotationConfig> = new Map();
  private currentKeyId: string | null = null;

  /**
   * Initialize key rotation with current key
   */
  initialize(currentKey: string): void {
   ...
   */
  rotateKey(retirementPeriodDays: number = 90): string {
  ...
  async reencryptData(
    ...
    oldKeyId: string
  ): Promise<{ data: string; keyId: string }> {
   ...
   */
  async reencrypt2FABackupCodes(userId: string, oldKeyId: string): Promise<void> {
    try {
      ...
      logger.error(`Failed to re-encrypt 2FA backup codes for user ${userId}`, 
   ...
   */
  async rotateAllEncryptedData(): Promise<{ 
    success: number; 
    failed: number; 
    total: number;
  }> {
    ...
    try {
      ...
      for (const user of usersWithencrypt2FA) {
        try {
          ...
          stats.failed++;
          logger.error(`Failed to rotate key for user ${user.id}`,
            error instanceof Error ? error : undefined
          );
        }
      }
      ...
      if (currentKeyId) {
        ...
        if (currentKey) {
          const daysSinceCreation = Math.floor(
            (Date.now() - currentKey.createdAt.getTime()) / (24 * 60 * 60 * 1000)
          );
