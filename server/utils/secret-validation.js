 ...
 */
export function validateSecret(secret, secretName, minLength = 32) {
    const issues = [];
    // Check length
...
export function validateCriticalSecrets() {
    ...
    if (process.env.TWO_FACTOR_BACKUP_ENCRYPTION_KEY) {
        const validation = validateSecret(process.env.TWO_FACTOR_BACKUP_ENCRYPTION_KEY, 'TWO_FACTOR_BACKUP_ENCRYPTION_KEY', 32);
