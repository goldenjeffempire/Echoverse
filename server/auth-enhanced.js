...
import { detectIpChange, detectUserAgentChange, generateDeviceFingerprint as genFingerprint } from "./utils/security";
 ...
 */
export async function recordLoginAttempt(identifier, ipAddress, userAgent, successful, failureReason) {
 ...
 */
export async function getRecentFailedAttempts(identifier, ipAddress) {
    const windowStart = new Date(Date.now() - LOGIN_ATTEMPT_WINDOW_MINUTES * 60 * 1000);
    const result = await db
        .select({ count: sql `count(*)` })
        .from(loginAttempts)
        .where(and(eq(loginAttempts.identifier, identifier), eq(loginAttempts.ipAddress, ipAddress), eq(loginAttempts.successful, false), gte(loginAttempts.attemptedAt, windowStart)));
...
export async function isAccountLocked(userId) {
    const lockout = await db
        ...
        .where(and(eq(accountLockouts.userId, userId), eq(accountLockouts.unlocked, false), gte(accountLockouts.lockedUntil, new Date())))
        .limit(1);
    if (lockout.length === 0) {
        return { locked: false };
    }
 ...
 */
export async function lockAccount(userId, reason, durationMinutes = LOCKOUT_DURATION_MINUTES) {
    const lockedUntil = new Date(Date.now() + durationMinutes * 60 * 1000);
    // Check if there's already an active lockout
    const existing = await db
        .select()
        .from(accountLockouts)
        .where(and(eq(accountLockouts.userId, userId), eq(accountLockouts.unlocked, false)))
        .limit(1);
 ...
 */
export async function checkAndLockIfNeeded(identifier, ipAddress, userId) {
    if (!userId)
        return;
    ...
    if (failedAttempts >= PROGRESSIVE_LOCKOUT_THRESHOLD) {
        // 10+ attempts = 24 hour lockout
        await lockAccount(userId, `Account locked for 24 hours due to ${failedAttempts} failed login attempts from IP ${ipAddress}`, EXTENDED_LOCKOUT_DURATION_HOURS * 60);
    }
    else if (failedAttempts >= MAX_LOGIN_ATTEMPTS) {
        // 5-9 attempts = 30 minute lockout
        await lockAccount(userId, `Account locked due to ${failedAttempts} failed login attempts from IP ${ipAddress}`, LOCKOUT_DURATION_MINUTES);
    }
}
/**
 ...
 */
export async function createPasswordResetToken(userId, ipAddress, userAgent, deviceFingerprint) {
 ...
 */
export async function validatePasswordResetToken(token, ipAddress, userAgent) {
    ...
    const result = await db
        ...
        .where(and(eq(passwordResetTokens.token, tokenHash), eq(passwordResetTokens.used, false), // Only update if not already used
...
export async function cleanupExpiredResetTokens() {
    const now = new Date();
    await db.delete(passwordResetTokens).where(and(sql `${passwordResetTokens.expiresAt} < ${now}`, eq(passwordResetTokens.used, false)));
}
/**
...
export async function cleanupExpiredLockouts() {
    ...
    await db
        ...
        .where(and(sql `${accountLockouts.lockedUntil} < ${now}`, eq(accountLockouts.unlocked, false)));
}
/**
...
export async function getUserSecurityStats(userId) {
    const [recentAttempts, lockouts, user] = await Promise.all([
        db
            .select()
            .from(loginAttempts)
            .where(and(sql `${loginAttempts.identifier} IN (SELECT username FROM ${users} WHERE id = ${userId})
              OR ${loginAttempts.identifier} IN (SELECT email FROM ${users} WHERE id = ${userId})`, gte(loginAttempts.attemptedAt, new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)) // Last 30 days
        ))
 ...
 */
export async function validateSessionSecurity(sessionId, currentIp, currentUserAgent, strictIpCheck = false) {
 ...
 */
export async function updateSessionActivity(sessionId, ipAddress, userAgent) {
    await db
        .update(sessions)
        .set({
 ...
 */
export async function createSecureSession(userId, ipAddress, userAgent, deviceType, expiryMs = 24 * 60 * 60 * 1000 // 24 hours default
) {
 ...
 */
export async function invalidateAllUserSessionsExcept(userId, exceptSessionId) {
    if (exceptSessionId) {
        await db
            .delete(sessions)
