...
class CostTracker {
     ...
     */
    async getStats(period = 'day', userId) {
        const now = new Date();
        const periodMs = {
            hour: 60 * 60 * 1000,
            day: 24 * 60 * 60 * 1000,
     ...
     */
    async sendCostAlert(userId, alert) {
        try {
     ...
     */
    async getCostAlert(threshold = 100, period = 'day', userId) {
     ...
     */
    async getCostBreakdown(userId, startDate, endDate) {
