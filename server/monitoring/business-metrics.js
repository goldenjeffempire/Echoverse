...
export function trackOrder(status) {
    orderCompletionRate.labels(status).inc();
}
export function updateRevenuePerUser(tier, revenue) {
    revenuePerUser.labels(tier).set(revenue);
}
export function trackCartAbandonment(abandoned) {
    if (abandoned) {
        cartAbandonmentRate.inc();
    }
}
export function trackUserSignup(source = 'direct') {
    userSignupsCounter.labels(source).inc();
}
export function trackAIRequest(provider, model, status) {
    aiRequestsCounter.labels(provider, model, status).inc();
}
export function updateAICost(provider, cost) {
    aiCostGauge.labels(provider).inc(cost);
}
export function trackContentCreation(type) {
