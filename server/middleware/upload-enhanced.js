 ...
 */
export function preValidateFileSize(req, res, next) {
    const contentLength = req.get('Content-Length');
    if (contentLength) {
        const size = parseInt(contentLength, 10);
        if (size > MAX_FILE_SIZE) {
            throw new AppError(413, `File size exceeds maximum allowed size of ${MAX_FILE_SIZE} bytes`, 'FILE_TOO_LARGE', { maxSize: MAX_FILE_SIZE, providedSize: size });
        }
    }
    next();
}
...
function validateFilename(filename) {
    ...
    if (filename.includes('..') || filename.includes('/') || filename.includes('\\')) {
        throw new AppError(400, 'Invalid filename: path traversal detected', 'INVALID_FILENAME');
    }
    // Prevent hidden files
    if (filename.startsWith('.')) {
        throw new AppError(400, 'Invalid filename: hidden files not allowed', 'INVALID_FILENAME');
    }
}
/**
    ...
    return (req, file, callback) => {
        try {
            ...
            if (!allowedTypes.includes(file.mimetype)) {
                ...
                return callback(new AppError(400, `File type not allowed: ${file.mimetype}`, 'INVALID_FILE_TYPE', { allowedTypes }));
            }
            callback(null, true);
        }
        catch (error) {
    ...
    try {
        ...
        if (!fileType) {
            ...
            if (!allowedTextTypes.includes(file.mimetype)) {
                await fs.unlink(file.path).catch(err => logger.error('Failed to delete rejected file', err));
                throw new AppError(400, 'Could not verify file type - file may be corrupted or invalid', 'INVALID_FILE_TYPE');
            }
        ...
        if (!allowedActualMimes) {
            ...
            await fs.unlink(file.path).catch(err => logger.error('Failed to delete rejected file', err));
            throw new AppError(400, `File type ${file.mimetype} not supported`, 'UNSUPPORTED_FILE_TYPE');
        }
        ...
        if (!allowedActualMimes.includes(fileType.mime)) {
            ...
            await fs.unlink(file.path).catch(err => logger.error('Failed to delete rejected file', err));
            throw new AppError(400, `File type validation failed: detected type (${fileType.mime}) does not match declared type (${file.mimetype})`, 'FILE_TYPE_MISMATCH', {
        ...
        logger.error('Magic byte verification failed', error);
        throw new AppError(500, 'File validation failed', 'FILE_VALIDATION_ERROR');
    }
}
/**
...
export async function postUploadValidation(req, res, next) {
    try {
        ...
        if (files && files.length > MAX_FILES) {
            throw new AppError(400, `Too many files: maximum ${MAX_FILES} files allowed`, 'TOO_MANY_FILES');
        }
 ...
 */
export function sanitizeFileMetadata(file) {
    return {
        filename: file.filename,
