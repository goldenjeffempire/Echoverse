...
export function validateRedirectUrl(redirectUrl) {
    try {
        ...
        const hasDangerousPattern = dangerousPatterns.some(pattern => pattern.test(redirectUrl));
 ...
 */
export function validateOAuthRedirect(req, res, next) {
    ...
    if (!validateRedirectUrl(redirectUrl)) {
        ...
        throw new AppError(400, 'Invalid redirect_uri. URL must be pre-registered.', 'INVALID_REDIRECT_URI');
    }
    // Store validated redirect URL for use in OAuth flow
    req.validatedRedirectUrl = redirectUrl;
    next();
}
/**
 ...
 */
export function validateOAuthState(receivedState, sessionState) {
 ...
 */
export function addAllowedRedirectUrl(url) {
