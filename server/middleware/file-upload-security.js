 ...
 */
export async function quarantineFile(filePath) {
    try {
 ...
 */
export async function postUploadSecurityValidation(req, res, next) {
    try {
        const file = req.file;
        ...
        for (const uploadedFile of filesToProcess) {
            ...
            if (!validateFilePath(filePath, UPLOAD_DIR)) {
                ...
                try {
                    ...
                    logger.error('Failed to delete suspicious file', unlinkError);
                }
            ...
            if (!scanResult.clean) {
                ...
                throw new AppError(400, `File rejected: ${scanResult.threats.join(', ')}`, 'SUSPICIOUS_FILE', { threats: scanResult.threats });
            }
            ...
            if (isFileEncryptionEnabled()) {
                try {
                    ...
                    throw new AppError(500, 'File encryption failed. Upload rejected for security reasons.', 'ENCRYPTION_FAILED', { originalError: error instanceof Error ? error.message : 'Unknown error' });
                }
            }
        }
        next();
 ...
 */
export function setSecureDownloadHeaders(res, filename, mimeType, forceDownload = false) {
 ...
 */
export function secureFileDownloadMiddleware(req, res, next) {
 ...
 */
export async function initializeQuarantineDirectory() {
    try {
        await fs.mkdir(QUARANTINE_DIR, { recursive: true });
