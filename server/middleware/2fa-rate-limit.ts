...
export const backupCodeRateLimit = rateLimit({
  ...
  handler: (req, res) => {
    ...
    throw new AppError(
      429,
      'Too many backup code attempts. Try again in 1 hour.',
      'BACKUP_CODE_RATE_LIMIT'
    );
  },
 ...
 */
export async function trackFailedTwoFAAttempt(
  userId: string,
  attemptType: '2fa-code' | 'backup-code'
): Promise<void> {
 ...
 */
export async function checkTwoFALockout(userId: string): Promise<boolean> {
 ...
 */
export async function resetTwoFALockout(userId: string): Promise<void> {
