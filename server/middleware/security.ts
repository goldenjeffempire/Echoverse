 ...
 */
function sanitizeGraphQLQuery(body: any): any {
 ...
 */
export function graphqlCsrfProtection(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  ...
  if (req.body && req.body.query && isGraphQLMutation(req.body.query)) {
    ...
    if (sessionId && process.env.SESSION_SECRET) {
      const isValid = verifyBoundCsrfToken(
        csrfTokenHeader,
        sessionId,
