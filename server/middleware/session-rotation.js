...
import { generateDeviceFingerprint, regenerateSessionId, validateSessionSecurity } from '../utils/session-security';
import { db } from '../db';
/**
...
export async function sessionRotationMiddleware(req, res, next) {
    ...
    try {
        ...
        if (shouldRotate) {
            ...
            try {
                ...
                const newSessionId = await regenerateSessionId(sessionId, userId, 'security_event', fingerprint);
 ...
 */
export function skipSessionRotation(req, res, next) {
    // Mark request to skip rotation
    req.skipSessionRotation = true;
    next();
}
/**
 ...
 */
export async function conditionalSessionRotation(req, res, next) {
    if (req.skipSessionRotation) {
        return next();
    }
 ...
 */
export async function rotateSessionToken(req, userId, privilegeChange = false) {
    ...
    try {
        ...
        const newSessionId = await regenerateSessionId(oldSessionId, String(userId), privilegeChange ? 'privilege_escalation' : 'security_event', fingerprint);
 ...
 */
export async function rotateSessionOnPrivilegeChange(userId, req, reason = 'privilege_change') {
    ...
    try {
        ...
        logger.error('Failed to rotate session', error, { userId, reason });
        throw error;
    }
}
