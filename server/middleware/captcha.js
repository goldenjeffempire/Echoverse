 ...
 */
export async function captchaMiddlewareLenient(req, res, next) {
    if (!CAPTCHA_ENABLED || CAPTCHA_PROVIDER === 'none') {
        return next();
    }
    const captchaToken = req.body?.captchaToken || req.headers['x-captcha-token'];
 ...
 */
export function skipCaptcha(req, res, next) {
 ...
 */
export async function conditionalCaptcha(req, res, next) {
    if (req.skipCaptcha) {
        return next();
    }
