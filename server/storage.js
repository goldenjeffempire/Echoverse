import { sessions, products, orders, users, websites, websiteVersions, posts, comments, communities, communityMembers, messages, campaigns, leads, plugins, pluginInstallations, auditLogs, notifications, media, abTests, abTestParticipants, funnels, funnelEntries, affiliates, referrals, userPermissions, passwordResetTokens, accountLockouts, aiCostTracking, webhookRetries, webhookEvents, refunds, forums, forumTopics, forumReplies, landingPages, shippingProviders, shippingRates, rssFeeds, websiteTemplates } from "@shared/schema";
import { db } from "./db";
...
export class PostgresStorage {
    ...
    async getForums(options) {
        ...
        const countQuery = whereClause
            ? db.select({ count: sql `count(*)::int` }).from(forums).where(whereClause)
            : db.select({ count: sql `count(*)::int` }).from(forums);
    ...
    async getForumTopics(forumId, options) {
        ...
        const countResult = await db.select({ count: sql `count(*)::int` }).from(forumTopics).where(eq(forumTopics.forumId, forumId));
        return { data, totalCount: countResult[0]?.count || 0 };
    }
    ...
    async getForumReplies(topicId, options) {
        const data = await db.select().from(forumReplies)
            .where(eq(forumReplies.topicId, topicId))
            .limit(options.limit || 50)
            .offset(options.offset || 0)
            .orderBy(forumReplies.createdAt);
        const countResult = await db.select({ count: sql `count(*)::int` }).from(forumReplies).where(eq(forumReplies.topicId, topicId));
        return { data, totalCount: countResult[0]?.count || 0 };
    }
    ...
    async getLandingPages(userId, options) {
        const data = await db.select().from(landingPages)
            .where(eq(landingPages.userId, userId))
            .limit(options.limit || 20)
            .offset(options.offset || 0)
            .orderBy(desc(landingPages.createdAt));
        const countResult = await db.select({ count: sql `count(*)::int` }).from(landingPages).where(eq(landingPages.userId, userId));
        return { data, totalCount: countResult[0]?.count || 0 };
    }
    ...
    // Funnels Management
    async getFunnels(userId, options) {
        const data = await db.select().from(funnels)
            .where(eq(funnels.userId, userId))
            .limit(options.limit || 20)
            .offset(options.offset || 0)
            .orderBy(desc(funnels.createdAt));
        const countResult = await db.select({ count: sql `count(*)::int` }).from(funnels).where(eq(funnels.userId, userId));
        return { data, totalCount: countResult[0]?.count || 0 };
    }
    async getFunnel(id) {
    ...
    async getFunnelEntries(funnelId, options) {
        ...
        const countResult = await db.select({ count: sql `count(*)::int` }).from(funnelEntries).where(eq(funnelEntries.funnelId, funnelId));
        return { data, totalCount: countResult[0]?.count || 0 };
    }
    async trackFunnelEntry(entryData) {
    ...
    async getAffiliates(options) {
        ...
        const countQuery = whereClause
            ? db.select({ count: sql `count(*)::int` }).from(affiliates).where(whereClause)
            : db.select({ count: sql `count(*)::int` }).from(affiliates);
        const countResult = await countQuery;
        return { data, totalCount: countResult[0]?.count || 0 };
    }
    ...
    async getReferrals(affiliateId, options) {
        ...
        const countResult = await db.select({ count: sql `count(*)::int` }).from(referrals).where(whereClause);
        return { data, totalCount: countResult[0]?.count || 0 };
    }
    ...
    async calculateShipping(address, weight) {
        ...
        const applicableRate = rates.find(r => weight >= (parseFloat(r.weightMin || '0')) &&
            weight <= (parseFloat(r.weightMax || '999999')));
        return applicableRate || rates[0];
    }
    // RSS Feed Management
    async getRssFeeds(userId) {
    ...
    async getAbTests(userId, options) {
        ...
        const countResult = await db.select({ count: sql `count(*)::int` }).from(abTests).where(whereClause);
        return { data, totalCount: countResult[0]?.count || 0 };
    }
    async getAbTest(id) {
    ...
    async trackAbTestParticipant(participantData) {
    ...
    async getAbTestResults(testId) {
        ...
        const variantStats = participants.reduce((acc, p) => {
    ...
    async getTemplates(options) {
        ...
        const countQuery = whereClause
            ? db.select({ count: sql `count(*)::int` }).from(websiteTemplates).where(whereClause)
            : db.select({ count: sql `count(*)::int` }).from(websiteTemplates);
        const countResult = await countQuery;
        return { data, totalCount: countResult[0]?.count || 0 };
    }
