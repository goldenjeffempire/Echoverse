...
const envSchema = z.object({
    ...
    ENABLE_VIRUS_SCAN: z.string().transform(val => val === 'true' || val === '1').default(process.env.NODE_ENV === 'production' ? 'true' : 'false'),
...
export function validateEnvironmentVariables() {
    ...
    if (!result.success) {
        ...
        const missingCritical = criticalSecrets.filter(key => errors[key] && errors[key].length > 0);
        ...
        if (process.env.NODE_ENV === 'production' || process.env.NODE_ENV === 'staging') {
            ...
            const missingProduction = productionSecrets.filter(key => !process.env[key] || (errors[key] && errors[key].length > 0));
