...
export function scaleConnectionPool(direction, reason) {
    const now = Date.now();
    ...
    if (direction === 'up') {
        currentPoolMax = Math.min(currentPoolMax + POOL_SCALE_CONFIG.scaleUpIncrement, POOL_SCALE_CONFIG.absoluteMax);
    }
    else {
        currentPoolMax = Math.max(currentPoolMax - POOL_SCALE_CONFIG.scaleDownDecrement, POOL_SCALE_CONFIG.absoluteMin);
    }
    if (currentPoolMax !== oldMax) {
        // Update pool configuration
        pool.options.max = currentPoolMax;
...
export function getCurrentPoolMax() {
