...
const breachCache = new Map<string, { breached: boolean; timestamp: number }>();
...
export async function validatePasswordSecurity(password: string): Promise<{
  ...
  if (errors.length === 0) {
    ...
    if (breachCheck.error) {
      ...
      errors.push(
        `This password has been exposed in ${breachCheck.count} data ${
          breachCheck.count === 1 ? 'breach' : 'breaches'
 ...
 */
export function getBreachCacheStats(): { size: number; entries: number } {
  return {
