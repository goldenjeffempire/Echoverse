...
class StripeWebhookHandler {
  private retryQueue: Map<string, WebhookRetry> = new Map();
  private readonly MAX_RETRIES = 5;
  private readonly INITIAL_BACKOFF_MS = 1000;
  private readonly MAX_BACKOFF_MS = 300000; // 5 minutes
  private readonly POISON_MESSAGE_THRESHOLD = 10;
  ...
  async handleEvent(event: Stripe.Event): Promise<void> {
    ...
    try {
      ...
      logger.error('Webhook event processing failed', error as Error, { eventId, type: event.type });
  ...
  private async scheduleRetry(eventId: string, error: Error): Promise<void> {
    ...
    if (retry.attempts >= this.POISON_MESSAGE_THRESHOLD) {
      ...
      try {
        ...
        logger.error('Failed to log dead letter event', dlqError as Error, { eventId });
      }
    ...
    const backoffMs = Math.min(
      this.INITIAL_BACKOFF_MS * Math.pow(2, retry.attempts - 1),
      this.MAX_BACKOFF_MS
    );
