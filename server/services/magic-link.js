...
export async function createMagicLink(userId, email, ipAddress) {
...
export async function validateMagicLink(token) {
    try {
        const result = await db
            ...
            .where(and(eq(magicLinkTokens.token, token), eq(magicLinkTokens.used, false), gte(magicLinkTokens.expiresAt, new Date())))
            .limit(1);
        if (result.length === 0) {
...
export async function cleanupExpiredMagicLinks() {
    const result = await db
        ...
        .where(and(eq(magicLinkTokens.used, true), gte(magicLinkTokens.expiresAt, new Date())))
