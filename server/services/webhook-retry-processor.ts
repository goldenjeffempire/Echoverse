...
export class WebhookRetryProcessor {
  ...
  async processRetries(): Promise<void> {
    ...
    try {
      ...
      await Promise.allSettled(
        retries.map(retry => this.processRetry(retry))
      );
    } catch (error) {
   ...
   */
  private async handlePoisonMessage(retry: any): Promise<void> {
   ...
   */
  private async deliverWebhook(webhookEventId: string): Promise<{
    success: boolean;
    error?: string;
    statusCode?: number;
  }> {
   ...
   */
  async scheduleRetry(webhookEventId: string, config?: Partial<WebhookRetryConfig>): Promise<void> {
