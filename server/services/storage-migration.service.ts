...
type StorageProvider = 's3' | 'gcs' | 'local';
 ...
 * Upload to AWS S3
 */
async function uploadToS3(key: string, options: UploadOptions): Promise<string> {
 ...
 */
async function uploadToGCS(key: string, options: UploadOptions): Promise<string> {
 ...
 */
function uploadToLocal(key: string, options: UploadOptions): string {
  const fs = require('fs');
...
export async function migrateLocalToCloud(): Promise<{
  ...
  const stats = { migrated: 0, failed: 0, errors: [] as string[] };
  const fs = require('fs');
  const path = require('path');

  async function migrateDirectory(dir: string, prefix = '') {
...
export function validateStorageConfig(): {
  ...
  if (STORAGE_CONFIG.provider === 's3') {
    if (!STORAGE_CONFIG.bucket) errors.push('AWS_S3_BUCKET not configured');
    if (!process.env.AWS_S3_ACCESS_KEY_ID) errors.push('AWS_S3_ACCESS_KEY_ID not configured');
    if (!process.env.AWS_S3_SECRET_ACCESS_KEY) errors.push('AWS_S3_SECRET_ACCESS_KEY not configured');
  }

  if (STORAGE_CONFIG.provider === 'gcs') {
    if (!STORAGE_CONFIG.bucket) errors.push('GCS_BUCKET not configured');
    if (!process.env.GCS_PROJECT_ID) errors.push('GCS_PROJECT_ID not configured');
    if (!process.env.GCS_CREDENTIALS) errors.push('GCS_CREDENTIALS not configured');
  }
