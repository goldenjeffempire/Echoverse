 ...
 */
async function queueEmailForRetry(options, retryCount) {
 ...
 */
async function moveEmailToDeadLetterQueue(options, error) {
 ...
 */
async function sendWithSES(options) {
    if (!sesClient) {
 ...
 */
export async function sendPasswordResetEmail(to, resetToken, expiresInMinutes) {
...
export function validateEmailConfig() {
    ...
    if (EMAIL_CONFIG.provider === 'ses') {
        if (!process.env.AWS_SES_REGION)
            errors.push('AWS_SES_REGION not configured');
        if (!process.env.AWS_SES_ACCESS_KEY_ID)
            errors.push('AWS_SES_ACCESS_KEY_ID not configured');
        if (!process.env.AWS_SES_SECRET_ACCESS_KEY)
            errors.push('AWS_SES_SECRET_ACCESS_KEY not configured');
    }
