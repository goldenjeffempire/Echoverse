...
// Dashboard overview
adminRouter.get('/dashboard/overview', authenticateToken, requireAdmin, async (req: AuthenticatedRequest, res) => {
  try {
    const [
      totalUsers,
      totalOrders,
      ...
      db.select({ total: sql<number>`sum(total)` }).from(orders).where(eq(orders.paymentStatus, 'paid')),
...
// User management
adminRouter.get('/users', authenticateToken, requireAdmin, async (req: AuthenticatedRequest, res) => {
  try {
...
adminRouter.patch('/users/:id/role', authenticateToken, adminRoleUpdateRateLimiter, requireAdmin, async (req: AuthenticatedRequest, res) => {
  try {
    const { role } = req.body;
...
// Analytics
adminRouter.get('/analytics', authenticateToken, requireAdmin, async (req: AuthenticatedRequest, res) => {
  try {
...
// Audit logs
adminRouter.get('/audit-logs', authenticateToken, requireAdmin, async (req: AuthenticatedRequest, res) => {
  try {
    const page = parseInt(req.query.page as string) || 1;
...
// System health
adminRouter.get('/system/health', authenticateToken, requireAdmin, async (req: AuthenticatedRequest, res) => {
  try {
...
adminRouter.get('/users/:id', authenticateToken, adminUserManagementRateLimiter, requireAdmin, async (req: AuthenticatedRequest, res) => {
  try {
    const [user] = await db
      .select({
        id: users.id,
...
adminRouter.patch('/users/:id', authenticateToken, adminUserManagementRateLimiter, requireAdmin, async (req: AuthenticatedRequest, res) => {
  try {
...
// Delete user (admin)
adminRouter.delete('/users/:id', authenticateToken, requireAdmin, async (req: AuthenticatedRequest, res) => {
  try {
...
// Product management
adminRouter.get('/products', authenticateToken, requireAdmin, async (req: AuthenticatedRequest, res) => {
  try {
    const page = parseInt(req.query.page as string) || 1;
    const limit = parseInt(req.query.limit as string) || 50;
    const offset = (page - 1) * limit;
...
// Order management
adminRouter.get('/orders', authenticateToken, requireAdmin, async (req: AuthenticatedRequest, res) => {
  try {
    const page = parseInt(req.query.page as string) || 1;
    const limit = parseInt(req.query.limit as string) || 50;
    const offset = (page - 1) * limit;
...
adminRouter.get('/system/config', authenticateToken, requireAdmin, async (req: AuthenticatedRequest, res) => {
  try {
    res.json({
...
adminRouter.get('/webhooks/failed', authenticateToken, requireAdmin, async (req: AuthenticatedRequest, res) => {
  try {
    const page = parseInt(req.query.page as string) || 1;
    const limit = parseInt(req.query.limit as string) || 50;
    const offset = (page - 1) * limit;
...
adminRouter.post('/webhooks/:webhookId/retry', authenticateToken, requireAdmin, csrfProtection, async (req: AuthenticatedRequest, res) => {
  try {
...
adminRouter.get('/webhooks/stats', authenticateToken, requireAdmin, async (req: AuthenticatedRequest, res) => {
  try {
    const [
      { totalRetries },
      { pendingRetries },
      ...
      db.select({ totalRetries: sql<number>`count(*)` }).from(webhookRetries).then((r: any) => r[0]),
      db.select({ pendingRetries: sql<number>`count(*)` }).from(webhookRetries).where(eq(webhookRetries.status, 'pending')).then((r: any) => r[0]),
      db.select({ failedRetries: sql<number>`count(*)` }).from(webhookRetries).where(eq(webhookRetries.status, 'failed')).then((r: any) => r[0])
    ]);
