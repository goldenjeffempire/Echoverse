...
import { passwordResetRateLimiter, twoFactorVerifyRateLimiter, twoFactorSetupRateLimiter, loginRateLimiter, registrationRateLimiter, emailVerificationRateLimiter, accountDeletionRateLimiter, passwordChangeRateLimiter, fileUploadRateLimiter, tokenRefreshRateLimiter, webhookRateLimiter, healthCheckRateLimiter, staticAssetRateLimiter, productCreationRateLimiter, mediaUploadRateLimiter, csrfTokenRateLimiter, backupCodesRateLimiter, adminQueryOperationsRateLimiter } from "./middleware/rate-limit-enhanced";
...
export async function registerRoutes(app) {
    ...
    app.put("/api/users/profile", authenticateToken, validateRequest(updateProfileSchema), async (req, res) => {
        try {
    ...
    app.post("/api/auth/change-password", authenticateToken, passwordChangeRateLimiter, validateRequest(changePasswordSchema), async (req, res) => {
        try {
    ...
    app.delete("/api/gdpr/delete-account", authenticateToken, accountDeletionRateLimiter, async (req, res) => {
        try {
    ...
    app.post("/api/create-payment-intent", authenticateToken, stripeIdempotencyMiddleware, validateRequest(createPaymentIntentSchema), async (req, res) => {
        try {
    ...
    app.post('/api/get-or-create-subscription', authenticateToken, stripeIdempotencyMiddleware, validateRequest(createSubscriptionSchema), async (req, res) => {
        const user = req.user;
    ...
    app.post('/api/webhooks/stripe', stripeIPWhitelistMiddleware, webhookRateLimiter, async (req, res) => {
        ...
        const verificationResult = await verifyStripeWebhookWithFallback(req.body, sig, stripe);
    ...
    app.post("/api/ai/advanced-generation", authenticateToken, aiRateLimiter, validateRequest(advancedGenerationSchema), async (req, res) => {
        ...
        try {
            ...
            let systemPrompt = `You are an expert ${type} creator. `;
            switch (complexity) {
                case 'advanced':
                    systemPrompt += 'Create sophisticated, production-ready content with advanced features, best practices, and optimizations.';
                    break;
                case 'expert':
                    systemPrompt += 'Create enterprise-grade content with cutting-edge features, security considerations, and scalability.';
                    break;
                default:
                    systemPrompt += 'Create high-quality, professional content with modern best practices.';
            }
            if (customizations) {
                systemPrompt += ` Apply these customizations: ${JSON.stringify(customizations)}`;
            }
            const result = await aiService.chatCompletion([
                ...
                temperature: 0.8,
                maxTokens: complexity === 'expert' ? 4000 : complexity === 'advanced' ? 3000 : 2000
            });
            res.json({
                success: true,
    ...
    app.post("/api/upload/image", authenticateToken, mediaUploadRateLimiter, (req, res) => {
    ...
    app.post("/api/upload/file", fileUploadRateLimiter, authenticateToken, (req, res) => {
    ...
    app.post("/api/orders/:id/refund", authenticateToken, requireRole(["admin", "moderator"]), async (req, res) => {
        try {
            ...
            if (order.stripePaymentIntentId && config.stripeSecretKey) {
                try {
                    const stripe = (await import('stripe')).default;
                    const stripeClient = new stripe(config.stripeSecretKey, { apiVersion: '2024-10-28.acacia' });
